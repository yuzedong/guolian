...
export default {
  ...
  // 输入数字、大小写字母和特殊字符
  noChinese2: (message = '请输入数字、大小写字母和特殊字符', trigger = ['blur', 'change']) => {
    const reg = /^[\n!#%+-=/\][{}|A-Za-z0-9]+$/
    // /^[\u4e00-\u9fa5]+$/
    const validator = (rule, value, callback) => {
      console.log(value)
  ...
  // 中文英文数字斜线
  cnEnNumPath: (message = '请输入中文、大小写英文、数字/、斜线', trigger = ['blur', 'change']) => {
    const reg = /^[/|\u4e00-\u9fa5|A-Za-z0-9]+$/
    const validator = (rule, value, callback) => {
      if (value === '' || reg.test(value)) {
        callback()
  ...
  // 中文英文数字 符号
  cnEnNumSymbol: (message = '请输入中文、大小写英文、数字、中文标点符', trigger = ['blur', 'change']) => {
    const reg = /^[、,，;；。'"|\u4e00-\u9fa5|A-Za-z0-9]+$/
    const validator = (rule, value, callback) => {
      if (value === '' || reg.test(value)) {
        callback()
  ...
  // 大于0整数
  greaterNumber: (message = '请输入大于0且合法的数字', trigger = ['blur', 'change']) => {
    const reg = /^[0-9]\d*$/
    const va = (rule, value, callback) => {
      if (value === '') {
        callback()
