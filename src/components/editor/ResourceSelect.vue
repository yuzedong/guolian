<template>
  <section class="resource-select-container">
    ...
    <el-dialog  :width="width" append-to-body :visible.sync="showFile" @close="closeDialog">
      <div v-if="fileType === 2">
        <video
          class="video"
          preload="auto"
      ...
      <div v-else-if="fileType === 1">
        <el-image class="t-img" :src="fileUrl" fit="contain">
  ...
  methods: {
    ...
    preview (row) {
      ...
      if (row.resourceType === 5) {
        ...
        if (this.fileType === 1) {
          this.width = row.width ? (Number(row.width) + 60) + 'px' : '600px'
