    ...
    <div class="base-table-wrap">
      <el-table
        ...
        :class="{
          draggable: draggable,
          'table-tree': isTree,
          'table-show-selection': showSelection,
          'table-show-index': 'showIndex',
        }"
        :data="data"
        :row-key="rowKey"
        :lazy="lazy"
      ...
      >
        ...
        <el-table-column
          v-if="showTag.flag"
          :label="$t(showTag.label) || ''"
          :fixed="isIndexFixed"
          :width="showTag.tagWidth"
        >
          <template slot-scope="scope">
            <template v-if="showTag.type === 'icon'">
              <jee-icon
                :iconClass="
                  scope.row[showTag.flagKey]
                    ? showTag.iconClassArr[0]
                    : showTag.iconClassArr[1]
                "
              ></jee-icon>
            </template>
        ...
        <template v-for="(column, index) in columns">
          ...
          >
            <template slot-scope="scope">
              <div
                :style="{ maxWidth: column.textWidth || '100%' }"
                :class="column.textWidth ? 't-overflow jee-color' : ''"
              >
                <a
                  v-if="column.target"
                  ...
                  >{{ getPropText(scope.row, column.prop) }}</a
                >
                <a
                  v-else
                  class="jee-font-second-light"
          ...
          >
            <template slot-scope="scope">
              <a
                class="jee-font-second-light"
                @click="
                  handleEvent(column.type, scope.row, scope.$index, $event)
                "
              >
          ...
          >
            <template slot-scope="scope">
              <span :class="{ 'is-disabled': scope.row.disabled }">{{
                column.formatter
                  ? column.formatter(
                      scope.row,
                      column,
        ...
        >
          <template slot-scope="scope">
            <div class="handle-column-wrap personclass">
              <template v-for="(item, index) in handleColumn">
                <template v-if="item.component === 'popover'">
                  <el-popover
                    ...
                    :popper-class="
                      item.popperClass
                        ? 'jee-popover--links' + ' ' + item.popperClass
                        : 'jee-popover--links'
                    "
                    placement="bottom"
                    trigger="hover"
                  ...
                  >
                    ...
                    <el-link slot="reference" :underline="false">
                      ...
                      {{ $t(item.name) }}
                      <i
                        class="el-icon-caret-bottom more-bottom"
                        style="font-size: 14px"
                      ></i>
                      <i
                        class="el-icon-caret-top more-top"
                        style="font-size: 14px"
                      ></i>
                    </el-link>
                  </el-popover>
  ...
  methods: {
    ...
    handleEvent(type, row, index, e, item) {
      ...
      if (item && item.href && (item.type === "link" || type === "Link")) {
        if (item.params) {
          this.$routerLink(item.href, item.type, {
            id: row[this.rowKey],
            ...item.params,
          });
        } else {
    ...
    rowDrop() {
      const tbody = document.querySelectorAll(
        ".base-table-wrap .el-table__body-wrapper tbody"
      )[this.dragIndex];
      const _this = this;
      // 修复火狐打开新tab并搜索问题
