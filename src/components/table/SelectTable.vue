        ...
        <template v-for="(column, index) in columns">
          ...
          >
            <template slot="header" slot-scope="scope">
              <span v-if="column.hiddenHeader">{{ column.label }}</span>
              <el-checkbox
                v-else
                :value="checkAll"
                :disabled="disabled"
            ...
            <template slot-scope="scope">
              <span v-if="column.hidden && column.hidden(scope.row, column)">{{
                column.formatter
                  ? column.formatter(
                      scope.row,
                      column,
          ...
          >
            <template slot="header">
              <el-checkbox
                :value="
                  checkColAll[column.prop] && checkColAll[column.prop].checkAll
                "
                :disabled="disabled"
                :indeterminate="
                  checkColAll[column.prop] &&
                  checkColAll[column.prop].isIndeterminate
                "
...
export default {
  ...
  computed: {
    checkAll() {
      ...
      return (
        list.length > 0 &&
        list.filter((d) => d.checkAllRow === false).length <= 0
      );
    },
  },
  methods: {
    ...
    handleCheckAllChange(scope, value) {
      this.$emit("handleCheckAllChange", !this.checkAll);
    },
    handleClick() {
      //   console.log('88')
    },
