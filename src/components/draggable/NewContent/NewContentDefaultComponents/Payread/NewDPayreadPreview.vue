<template>
    <section
        class="d-payred-preview z-draggable-preview new-content payread-content"
    >
        <el-form-item
            :label="option.label"
            :prop="option.name"
        ...
        >
            ...
            <div
                v-if="form[option.name] && form[option.name].payRead"
                class="inpParent"
            >
                <el-input
                    class="inp"
                    @input="changeInput"
                    v-model="form[option.name].payPrice"
                    maxlength="8"
                    placeholder="请输入售价(0.01-10000)"
                ></el-input>
                <el-input
                    class="inp laoer"
                    v-model="form[option.name].trialReading"
                    maxlength="5"
                    placeholder="请输入试读字数(0-99999)"
                ></el-input>
            </div>

            <div class="z-tip-form-item" v-if="option.tip">
                {{ option.tip }}
            </div>
        </el-form-item>
    </section>
      ...
      this.$request.fetchPayConfig().then(res => {
        if (res.code == 200) {
          ...
          if (!this.form[this.option.name].trialReading) {
            this.form[this.option.name].trialReading = res.data
              .payRead
              ? res.data.payRead
              : ''
          }
          if (!this.form[this.option.name].payPrice) {
            this.form[this.option.name].payPrice = res.data.payPrice
              ? String(res.data.payPrice)
              : ''
          }
