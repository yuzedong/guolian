            ...
            <template v-for="(option, index) in scope.form.options || []">
              <div class="option-item new-cont-option flex" :key="index" >
                <el-radio :label="option.value">
                  <span v-if="scope.form.disRadioBtn">{{option.label}}</span>
                  <div v-else class="option-radio option-radio-gender">
                    <el-input v-model="scope.form.options[index].label" size="medium" :maxlength="50"></el-input>
                  </div>
                </el-radio>
                <div class="icon flex">
                  <jee-icon  v-if="!scope.form.disRadioBtn" iconClass="jia1" class="jee-hover-fill"  @click="handleAddOption(index)"></jee-icon>
                  <jee-icon v-if="!scope.form.disRadioBtn" iconClass="jianqu" class="jee-hover-fill" @click="()=>handleDelOption(index)"></jee-icon>
                  <jee-icon iconClass="tuozhuai" class="jee-hover-fill move"></jee-icon>
                </div>
              </div>
            </template>
          </draggable>
          <el-button v-show="!scope.form.options.length&&!scope.form.disRadioBtn" class="btn-add btn-default" type="empty" @click="handleAddOption">
            <jee-icon iconClass="jia1"></jee-icon>
            <span>添加选项</span>
          </el-button>
          <div class="other-option-wrap" v-show="!scope.form.disRadioBtn">
            ...
            <transition name="fade">
              <div v-if="scope.form.isOtherOption" class="other-option">
                <el-input v-model="scope.form.otherOptionLabel" size="medium" :maxlength="50"></el-input>
