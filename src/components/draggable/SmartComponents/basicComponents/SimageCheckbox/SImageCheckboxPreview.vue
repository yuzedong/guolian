<template>
  <section class="s-image-radio-previews s-draggable-preview">
    ...
    >
      ...
      <!-- :style="deviceTab === 2?{width: (option.column===1?'25%':option.column===2?'50%':option.column===3?'75%':'100%'),
         marginRight: '0'}:{width: '50%', marginRight: '0'} " -->
          ...
          <template v-for="(opt, index) in getOptions || []">
            <div
              ...
              :style="(index + 1) % option.column==0?{
                      marginRight:0+(((contWidth*0.8-50)/190)-option.column)*140 +'px'}:{}"
            >
              <div>
                <div class="radio-box ">
                  <el-image
                  ...
                  >
                    ...
                    >
                      <jee-icon
                        style="font-size: 27px;fill: #B3B3B3;"
                        iconClass='morentupian'
                      ></jee-icon>
                    </div>
                  </el-image>
                  <el-checkbox
      ...
      <div
        class="z-tip-form-item"
        v-if="option.tip"
      >{{option.tip}}</div>
    </el-form-item>
    <el-form-item
      v-else
      class="smart-mobile-box"
      :label="option.label"
      :prop="option.name"
      :rules="getRules"
    >
      <div class="el-image-radio-group">
        <template v-for="(opt, index) in getOptions || []">
          <div
            :key="index"
            :class="opt.picUrl?'t-inline pic-none-border':'t-inline'"
          >
            <el-checkbox-group
              class="radio-box "
  ...
  methods: {
    setColor (item) {
      const inputs = document.querySelectorAll(
        '.s-image-radio-preview .radio-label'
      )
      const options = document.querySelectorAll(
        '.s-image-radio-preview .radio-item .el-radio__label'
      )
