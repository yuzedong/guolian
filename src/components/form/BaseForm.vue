<template>
  <section class="base-form-container">
    <el-form
      ...
      <el-form-item v-if="showSubmitButton" class="item-btn"
        :label-width="btnLabelWidth||labelWidth"
        :class="submitBtns[0]?submitBtns[0].class:''">
        <label v-if="btnText" class="btn-label" style="margin-right:10px">{{btnText}}</label>
        <div v-if="submitBtnsLength" class="t-inline">
...
export default {
  ...
  methods: {
    ...
    handleSubmit (e, btn) {
      if (btn.subType === 'Del') {
        this.$emit(`handleConfirm${btn.subType}`, this.form, btn)
        return
      }
