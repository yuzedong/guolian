...
export default {
  ...
  methods: {
    ...
    uploadType (file) {
      ...
      this.suffix = type.slice(i + 1).toLowerCase()
    ...
    handleUpload () {
      ...
      if (this.autoUpload) {
        ...
        // 修复水印图片在上传时被添加水印
        console.log(this.uploadOption.mark)
        if (this.uploadOption.mark === false) {
    ...
    beforeImageUpload (file) {
      ...
      this.suffix = file.name.substring(i + 1).toLowerCase()
      const allowTypes = this.type instanceof Array ? this.type : (this.type ? this.type.split(',') : [])
