    ...
    >
      ...
      >
        <span
          :class="{
            'jee-color':
              node.isCurrent || (node.level && node.level === 1 && showLevel),
          }"
          >{{ data[defaultProps.label || "label"] }}</span
        >
        <i
          class="el-icon-remove-outline jee-color"
          v-if="node.expanded && !data.leaf"
          @click="() => handleCollect(node, data)"
        ></i>
        <i
          class="el-icon-circle-plus-outline jee-color"
          v-if="!node.expanded && !data.leaf"
          @click="() => handleExpand(node, data)"
        ></i>
        <div class="sort-wrap">
          <jee-icon
            iconClass="ziyuanldpi"
            class="jee-hover-fill"
            v-if="data.prevId"
            @click.stop="handleSort(data, 'up')"
          />
          <jee-icon
            iconClass="ziyuanldpi1"
            class="jee-hover-fill"
            v-if="data.nextId"
            @click.stop="handleSort(data, 'down')"
          />
        </div>
      </span>
    </el-tree>
