...
export default {
  ...
  state: {
    ...
    lang: '',
    exportPrintFlag: true, // 判断是否有框架头部和侧边栏
    langs: [
      {
        name: 'cn',
        label: '中文(简体)'
  ...
  mutations: {
    ...
    // 判断是否有框架头部和侧边栏
    UPDATE_EXPORT_PRINT_FlAG (state, data) {
      state.exportPrintFlag = data
    },
    // 用户和路由权限
    SET_USER (state, data) {
      ...
      if (localStorage.getItem('userName') && localStorage.getItem('userName') !== user.userName) {
        ...
        window.localStorage.setItem('userUpdate', true)
      } else {
        window.localStorage.setItem('userName', user.userName || '')
        window.localStorage.setItem('userUpdate', false)
      }
  ...
  actions: {
    ...
    changeExportPrintFlag ({ state, commit, rootState, dispatch }, data) {
      commit('UPDATE_EXPORT_PRINT_FlAG', data)
    },
    // 普通登录
