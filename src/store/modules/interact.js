...
export default {
  ...
  mutations: {
    ...
    GET_MAIL_DATA: (state, data) => {
      let arr = ['bgConfig', 'headConfig', 'contConfig', 'componentConfig', 'subConfig']
      let obj = data
      arr.forEach((t, i) => {
        obj[t] = obj[t] ? (data[t] instanceof Object ? data[t] : JSON.parse(data[t])) : state.smartSkin[t]
      })
      state.mailBoxDetail = obj
    },
    GET_CLOSE_DATA: (state, data) => {
      state.closeLayerState = data
    },
    GET_MAILBOX_LETTER_TYPES: (state, data) => {
      state.mailboxLetterTypes = data
    },
    GET_MAILBOX_LETTER_BOXS: (state, data) => {
      state.mailboxLetterBoxs = data
    }
  },
  getters: {
  ...
  actions: {
    ...
    resetMailDetailData ({ commit, state }, data) {
      commit('GET_MAIL_DATA', data)
    },
    ...
    getMailBoxDetailData ({ commit, state }, params) {
      let { id, type } = params
      if (!state.mailBoxDetail.id || type) {
        request.fetchMailboxFormGet({ id }).then(res => {
          if (res.code === 200) {
            commit('GET_MAIL_DATA', res.data)
          }
        })
      }
    },
    ...
    // 获取信件类型
    getMailboxLetterTypes ({ commit }, params) {
      request.fetchMailboxLetterTypeGetPage(params).then(res => {
        if (res.code === 200) {
          commit('GET_MAILBOX_LETTER_TYPES', res.data.content)
        }
      })
    },
    // 获取可管理信箱列表
    getMailboxLetterBoxs ({ commit }, params) {
      request.fetchMailboxLetterBoxGetMyboxs(params).then(res => {
        if (res.code === 200) {
          commit('GET_MAILBOX_LETTER_BOXS', res.data)
        }
      })
    }
  }
