...
export default {
  ...
  state: {
    ...
    onlyContentSecurityOptions: [], // 内容密级下拉选项-仅内容
  ...
  mutations: {
    ...
    // 设置内容密级下拉选项数据-仅内容
    SET_OCSO: (state, data) => {
      state.onlyContentSecurityOptions = loopOptions(data)
    },
    // 设置附件密级下拉选项数据
    SET_ASO: (state, data) => {
  ...
  actions: {
    ...
    // 请求内容密级下拉选项-仅内容
    FetchOnlyContentSecurityOptions ({ commit, state }, reflush) {
      if (!state.onlyContentSecurityOptions.length || reflush) {
        request.fetchSysSecretsContentAll({ secretType: 1 }).then(res => {
          if (res.code === 200 && res.data instanceof Array) {
            commit('SET_OCSO', res.data)
          } else {
            commit('SET_OCSO', [])
          }
        })
