<template>
  ...
  <global-layout>
    <!-- 标签上的右键菜单 -->
    <ContextMenu ref="contextMenu" v-if="exportPrintFlag" :itemList="menuItemList" @mouseleave="onMouseLeave" :visible.sync="menuVisible" @select="onMenuSelect" />
    <!-- 标签页tabs -->
    <el-tabs
      class="menus-tabs-container"
      type="card"
      closable
      v-if="exportPrintFlag"
      v-model="activePage"
...
export default {
      ...
      filterMenus: [
        {
          ...
          link: '/interact/smart/detail',
          text: '检测到您打开了智能表单编辑页，您的修改尚未保存，确认关闭吗？'
        },
        {
          name: 'interactMailboxFormDetail',
          link: '/interact/mailbox/form/detail',
          text: '检测到您打开了领导信箱表单编辑页，您的修改尚未保存，确认关闭吗？'
        }
      ]
    }
  },
  ...
  watch: {
    ...
    pageList (newData, oldData) {
      ...
      let filterObj = newData.find(item => this.filterMenus.find(t => t.name === item.name))
      window.onbeforeunload = filterObj ? e => {
        e.preventDefault()
        e.returnValue = ''
      } : null
    }
  ...
  methods: {
    ...
    removeTab (key) {
      ...
      let menuKey = key.split('?')[0]
      let filterObj = this.filterMenus.find(t => t.link === menuKey)
      if (filterObj && this.closeLayerState) {
        if (!confirm(filterObj.text)) return
      }
    ...
    //   console.log(key, pageKey)
      // 智能表单 领导信箱表单
      let menuKey = pageKey.split('?')[0]

      let filterPageObj = this.pageList.find(item => this.filterMenus.find(t => t.name === item.name))
      if (filterPageObj) {
        menuKey = filterPageObj.path
      }
      let filterObj = this.filterMenus.find(t => t.link === menuKey)
      ...
      if (key === '3' && (filterObj && menuKey === filterObj.link)) {
        closeStatus = true
      }
      if (filterObj && !confirm(filterObj.text)) { closeStatus = false } else if (filterObj && confirm(filterObj.text)) {
        closeStatus = true
      }
