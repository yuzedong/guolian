...
export default {
  ...
  data() {
    return {
      ...
      headers: {
        ...
        showAlertIcon: true,
        content:
          "系统中只能存在一个顶层组织，该组织拥有最高权限。顶层组织可以修改信息，但不能删除和修改权限；",
      },
      count: 0,
      showTable: false,
    };
  ...
  mounted() {
    ...
    this.$request.fetchOrganizeCount().then((res) => {
      if (res.code === 200) {
        this.list.api = "fetchOrganizeList";
        if (res.data > 100) {
          this.list.lazy = true;
          this.list.params.getAll = false;
        } else {
          this.list.lazy = false;
          this.list.params.getAll = true;
        }
        this.fetchTableApi();
      }
    });
  ...
  watch: {
    "list.selectedKeys": function (newData) {
      ...
      this.headers.buttons[1].disabled = !this._checkPermission(
        "/orgs",
        "DELETE"
      );
    },
