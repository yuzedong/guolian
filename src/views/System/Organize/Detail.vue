  ...
  <section class="org-detail-container left-container">
    <el-container>
      ...
      <el-main style="padding: 10px">
        ...
        />
        <base-form v-bind="form" v-on:handleConfirm="handleConfirm"></base-form>
        <msg-dialog
          v-bind="msgDialog"
...
export default {
    ...
    return {
      ...
      form: {
        ...
        rules: {
          ...
          phone: [this.$rules.phoneAll()],
          orgFax: [this.$rules.phone("请输入正确的传真号码")],
        },
        submitBtns: [
          {
            text: "保存",
  ...
  methods: {
    ...
    handleHeaderDelete() {
      ...
      this.msgDialog.content =
        "删除组织时会删除下级所有组织，以及组织内的角色及用户，是否确定删除？";
    ...
    handleConfirm(data) {
      ...
      if (
        params.orgids.length > 0 &&
        params.orgids[params.orgids.length - 1] !== "parent"
      ) {
    ...
    fetchFormCallBack(res) {
      if (res.code === 200) {
        ...
        this.form.formItems.find((d) => d.prop === "orgids").disabled =
          !res.data.parentAble;
        this.headers.buttons.find((d) => d.type === "Delete").disabled =
          !res.data.deleteAble;
        this.form.submitBtns.find((d) => d.subType === "submit").disabled =
          !res.data.managerAble;
        this.getTreeList();
        // managerAble 是否可管理
      }
    },
    getTreeList() {
      if (this.organizeOptions instanceof Array && !this.lazy) {
        ...
        if (!this.form.form.parentId) {
          ...
          this.form.formItems.find((d) => d.prop === "orgids").options =
            options;
          ...
          this.form.formItems.find((d) => d.prop === "orgids").options =
            loopopt(deepClone(this.organizeOptions));
        }
