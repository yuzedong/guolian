...
export default {
  ...
  methods: {
    ...
    getInitDefaultConfig () {
      return defaultConfigs.slice().sort((a, b) => a.index - b.index).map(d => {
        const key = Date.now() + '_' + Math.ceil(Math.random() * 1000000)
        d.prop = d.isCustom?d.type:d.type + '_' + key
        return d
      })
      // defaultConfigs.filter(d => !.includes(d.type)).slice().sort((a, b) => a.groupIndex - b.groupIndex)
    },
    getDefaultJson (list) {
      let arr1 = this.getInitDefaultConfig()
      let arr2 = this.data.formListBase
      ...
      let types = arr1.map((t) => {
        loop(arr2, t.type, t.editor)
        return t.type
      })
    //   console.log(types)
      ...
      let arr5 = arr1.filter(d => !arr4.includes(d.type)).slice().sort((a, b) => a.groupIndex - b.groupIndex)
