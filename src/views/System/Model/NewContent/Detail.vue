...
export default {
  ...
  methods: {
      ...
      this.$request.fetchModelDetail({ id: this.modelId }).then(res => {
        if (res.code === 200) {
          ...
          if (!(enableJson instanceof Object && Object.keys(enableJson).length)) {
            ...
            // console.log(JSON.stringify(formListBase[1]))
            // console.log(JSON.stringify(formListBase[2]))
    ...
    getDefaultJson (list) {
      ...
      //   let arr2 = this.getInitDefaultConfig()
      let arr4 = []
      let arr6 = []
      let loop = function (arr, editor, preview) {
        arr.forEach((t) => {
          if (t.editor === editor) {
            arr4.push(preview)
          }
          if (t.list instanceof Array) {
            loop(t.list, preview, editor)
          }
        })
      }
      arr2.forEach((t) => {
        loop(this.data.formListBase, t.editor, t.preview)
        loop(this.data.formListExtend, t.editor, t.preview)
      })
      //   console.log(types)
      // arr6 = arr2.filter(d => !types.includes(d.type)).slice()
      let arr5 = arr2.filter(d => !arr4.includes(d.preview)).slice().sort((a, b) => a.groupIndex - b.groupIndex)
...
.column-model-main-container{
  ...
  .content-block-right{
    ...
    .d-checkbox-preview.z-draggable-preview{
        >.el-form-item{
            padding: 8px 30px 20px;
        }
    }
