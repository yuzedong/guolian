...
export default {
  ...
  methods: {
    ...
    async handleConfirm (data) {
      ...
      let baseArr = ['telephone', 'usePhone', 'username', 'realname']
      baseArr.forEach(t => {
        if (params[t]) {
          params[t] = this.$encode2(params[t])
        }
      })
      delete params.org
      delete params.orgId
      delete params.userExt
      this.$request.fetchSysUsersUpdate(params).then(res => {
        if (res.code === 200) {
          this.$message({
            message: '保存成功',
            type: 'success'
