<template>
  <section class="district-content-index">
    ...
    <template #sortNum="scope">
      <el-input type="number" v-model="scope.scope.row.sortNum" maxlength="11" size='mini' min="1" max="99999999"
...
export default {
  ...
  methods: {
    ...
    rowBlur (data) {
      if (data.sortNum) {
        if (data.sortNum < 1 || data.sortNum > 99999999) {
          this.$message.error('请输入1~99999999之间的值')
          return
        }
        let sorts = []
        let obj = {}
