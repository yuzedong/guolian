<template>
<!-- 链接地址详情列表-->
  <section class="interact-smart-link-ques">
    <base-form
      v-bind="form"
    ...
    >
      ...
      <template #qrCode="scope">
        <div class="code">
          <img class="t-img" :src='form.form.qrCodeUrl' alt="">
        </div>
...
export default {
  ...
  methods: {
    ...
    getLinkDetail () {
      ...
      this.$request.fetchSmartFormDetail({
        'id': this.$route.query.id
      }).then(res => {
        if (res.code === 200) {
          this.form.form = Object.assign(this.form.form, res.data)
          this.form.form.qrCodeUrl = `${this.$path}/common/qrcode/360?val=${this.form.form.url}`
          this.form.form.iframe = "<iframe src='" + res.data.url + "' width='799' height='800' frameborder='0' style='overflow:auto'></iframe>"
          this.form.form.embedded = res.data.callScript
        }
      })
    },
    // 下载二维码
    getQrcode (size) {
      let url = `${this.$path}/common/qrcode/${size}?val=${this.form.form.url}`
      this.getBase64(url, size)
    },
    getBase64 (imgUrl, size) {
      var image = new Image()
      // 解决跨域 Canvas 污染问题
      ...
      image.onload = function () {
        ...
        a.download = size + 'PX*' + size + 'PX.jpeg' // 设置图片名称
    ...
    getBlob (base64, size) {
      ...
      this.$exportClick(bdata, size + 'PX*' + size + 'PX.jpeg')
    },
    bigImg () {
      this.getQrcode(1080)
    },
  ...
  mounted () {
    if (this.$route.query.id && this.name === 'linkAddress') {
      this.getLinkDetail()
    }
  },
  activated () {
