              ...
              if (data instanceof Array && data.length) {
                return data.map((d) => {
                  if (d.ops instanceof Array && d.ops.length) {
                    d.ops.forEach((row) => {
                      // 操作对象转为勾选框+禁用状态
                      const rd = that.selectList.columns.find(
                        (c) => c.operation === row.operation
                      );
                      if (rd) {
  ...
  mounted() {
    if (this.name && this.value) {
      ...
      this.$request.fetchOrganizeCount().then((res) => {
        if (res.code === 200) {
          this.selectList.api = "fetchChannelPermOrg";
          // 暂时懒加载后获取不到完整数据，等实现后在改为>100
          if (res.data > 10000000) {
