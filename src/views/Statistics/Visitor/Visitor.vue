...
import { getTime, getData, transverterMss } from '@/utils'
const visitorObjs = {
  newName: '新客户',
  newPv: 0,
  newUv: 0,
  ...
  methods: {
    ...
    getSummaries (param) {
      ...
      columns.forEach((column, index) => {
        ...
        if (this.collect) {
          if (column.property === 'visitTime') {
            sums[index] = this.collect.visitTimes
          } else if (column.property === 'jump') {
            sums[index] = this.collect[column.property] + '%'
          } else if (column.property) {
            sums[index] = this.collect[column.property]
            console.log(this.collect[column.property])
          } else {
            sums[index] = 'N/A'
          }
        }
