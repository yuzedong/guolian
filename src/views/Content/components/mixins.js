...
export default {
  ...
  watch: {
    '$route': function (newRoute, oldRoute) {
      ...
      const { name, query } = newRoute
      const { channelId, type, modelId, id } = query
      ...
      } else if (name === 'contentCreate') {
        if (type === 'add' && (Number(modelId) !== Number(this.modelId) || Number(channelId) !== Number(this.channelId))) {
          console.log('xinz')
          this.detail = {}
