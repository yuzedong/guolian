                          ...
                          <template v-for="col in row">
                            <template v-for="(cell, cellIndex) in col">
                              ...
                              <div class="widget-view edit" :key="cell.prop" :class="{'model-left-list': cell.list}" style="width: 50%; box-sizing: border-box;">
                                ...
                                <component v-else-if="cell.list === undefined&&isShowPreview(cell.preview)" :is="cell.preview" :index="cellIndex" :option="cell.value" :form="form" v-model="form[cell.value.name]"
                                    ...
                                    <template v-for="(item, itemIndex) in cell.list">
                                      <div class="ghost_list_box" :key="itemIndex" :class="{'flex50 hb': item.type === 'fileUpload' || item.type === 'multiImageUpload', w100: item.type !== 'fileUpload' && item.type !== 'multiImageUpload'}">
                                        <div class="widget-view edit" :key="item.prop">
                                          <component v-if="isShowPreview(item.preview)" :is="item.preview" :index="itemIndex" :option="item.value" :form="form" v-model="form[item.value.name]"
                    ...
                    <el-tabs type="card" class="tab-document" v-model="activeDocument" v-else-if="row.type === 'document'" :before-leave="documentChange">
                      ...
                      <el-tab-pane :label="row.name" :name="row.type">
                        <div class="widget-view edit" :key="row.prop">
                          <component  v-if="isShowPreview(row.preview)" :is="row.preview" :index="rowIndex" :option="row.value" :form="form" v-model="form[row.value.name]"
                        ...
                        <div class="model-left-container">
                          <div class="model-left-label">
                            <div class="widget-view edit" :key="row.prop">
                              ...
                              <component v-else-if="isShowPreview(row.preview)" :is="row.preview" :index="rowIndex" :option="row.value" :form="form" v-model="form[row.value.name]"
                                         :isValidator='true' :getContentText="getContentText"></component>
                            </div>
                          </div>
                        </div>
                            ...
                            <div class="model-left-label flexwrap">
                              <template v-for="(col, colIndex) in row.list">
                                <div class="widget-view edit" :class="{flex50: row.type === 'fileUpload' || row.type === 'multiImageUpload', w100: row.type !== 'fileUpload' && row.type !== 'multiImageUpload'}" :key="col.prop">
                                  <component v-if="isShowPreview(col.preview)" :is="col.preview" :index="colIndex" :option="col.value" :form="form" v-model="form[col.value.name]"
            ...
            <div class="content-block-right" v-show="detail.formListExtend && detail.formListExtend.length > 0">
              <template v-for="(item, index) in detail.formListExtend">
                ...
                <div class="ghost-box" v-else :key="index" >
                  <div class="widget-view edit"  :key="item.prop" >
                    <component v-if="isShowPreview(item.preview)" :is="item.preview" :index="index" :option="item.value" :form="form" v-model="form[item.value.name]"
                               :isValidator='true' :getContentText="getContentText"></component>
                  </div>
                </div>
              </template>
...
export default {
  ...
  methods: {
    ...
    getContentText () {
      ...
      // console.log(this.filterHtml(this.$refs[this.activeContentTab]));
    },
    filterHtml (str) {
    ...
    // 获取新增字段
    fetchContentDetailAdd  (modelId) {
      var _this = this
      _this.loading = true
      _this.$request.fetchContentPlus(modelId).then(res => {
        if (res.code === 200) {
          _this.detail = {
    ...
    handleSubmit () {
      this.$refs.form.validate(valid => {
        ...
        //   params.reward = 0
        // }
        // if (params.payRead.payRead=="null") {
        //   params.payRead.payRead=0
        // }
    ...
    getFormDatas (status) {
      var { id, channelId, isQuote, type, isCited, modelId } = this.$route.query
      ...
      if (type !== 'add') {
        ...
        this.fetchContentDetailAdd(modelId)
      }
    },
...
.column-model-edit-container.content-detail-container{
  .content-block-box{
    ...
    .d-check-preview.z-draggable-preview{
        >.el-form-item{
            padding: 8px 30px 20px;
        }
    }
